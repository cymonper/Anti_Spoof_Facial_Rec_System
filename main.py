...
class IdData:
    def __init__(
        ...
        os.system('cls')
        print("\n    Name : Cymon Pereira \n    ID: 19694175\n\n\n\n\n\n")
        print("\n The %d face images are placed in authorized users folder. \n In the current project I have saved the authorised persons face images as my name, that is Cymon. \n \n Now loading images from the folder into the model, Once the images are loaded the Frame will be launched.... \n" % len(image_paths))
    ...
    def detect_id_faces(self, image_paths):
        ...
        for image_path in image_paths:
            ...
            face_patches, _, _ = main2.detect_faces(image, self.mtcnn)
...
def load_model(model):
    ...
    if os.path.isfile(model_exp):
        print("Model Initialized: %s" % model_exp)
...
def main(args):
    with tf.Graph().as_default():
        with tf.Session() as sess:
            mtcnn = main2.create_mtcnn(sess, None)
            ...
            while True:
                ...
                face_patches, padded_bounding_boxes, landmarks = main2.detect_faces(frame, mtcnn)
                ...
                if len(face_patches) > 0:
                    ...
                    for bb, landmark, matching_id, dist in zip(
                        ...
                        if matching_id is None:
                            ...
                            print("Face recognised as: %s At distance: %1.4f \n" % (matching_id, dist))
                            cap.release()
                            os.system('python blink_detection.py') 
                            sys.exit()
                    ...
                    print("No face infront of the camera")
                end = time.time()
                seconds = end - start
